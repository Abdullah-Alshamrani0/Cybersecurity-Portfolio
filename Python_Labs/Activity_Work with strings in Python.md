...
the data type of the "employee_id" variable was displayed 'int' as a data type the first time, after that the "employee_id" variable got reassigned and it displays 'str' data type instead.
...
The code outputs ".com", because the value of ind has been set to index ".com" and the code prints the start of ind plus 4 letters after the initial index
...
with python you can index and slice strings, and that helps make things a lot easier for checking and validating websites for example, where you can check if the website being used is safe by checking if the website is using http or https protocol
