...
After sorting the failed login attempts list, it appears that the number of failed log in attempts is increasing.
...
The highest amount of login attempts from the failed login attempt list is 264
...
Calling the function above displays the total of logins made in the same day by the user "ejones", and the output would vary for different users 
...
This version of analyze_logins() displays the username and the current day logins and the average day logins, and the output would vary for different users depending the on the input provided to the function
...
This version of the function returns the login ratio, and that allows the login ratio to be used later in the program
...
Using fuctions makes it easier to do repatitive tasks, and when the fuction is edited or change, the change applies everywhere else where the fuction is being used, and that's more efficient than retyping the same lines of code whenever you need to do the same process
